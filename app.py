...
# ====== NOUVELLE FONCTION POUR LA RECHERCHE SIMPLIFIÉE ======
def show_simplified_search():
    """Affiche l'interface de recherche simplifiée"""
    try:
        from modules import recherche_simplified
        
        # Indicateur de version
        st.markdown('<div class="version-indicator">🧪 VERSION SIMPLIFIÉE (BETA)</div>', unsafe_allow_html=True)
        
        # Afficher l'interface simplifiée
        recherche_simplified.show_page()
        
    except ImportError as e:
        st.error("❌ Module recherche_simplified.py non trouvé !")
        st.info("Assurez-vous d'avoir créé le fichier modules/recherche_simplified.py")
        with st.expander("Détails de l'erreur"):
            st.code(str(e))
    except Exception as e:
        st.error(f"❌ Erreur lors du chargement de la recherche simplifiée: {str(e)}")
        with st.expander("Détails de l'erreur"):
...
def main():
    ...
    with st.sidebar:
        ...
        use_simplified = st.toggle(
            ...
            help="Active la version simplifiée sans redondances"
        )
        ...
        if use_simplified:
            st.info("✨ Version simplifiée active")
            st.caption("Cette version élimine les redondances et optimise l'architecture")
        else:
            st.info("📋 Version classique active")
            st.caption("Version originale avec toutes les fonctionnalités")
        ...
        with st.expander("📚 Aide"):
            if use_simplified:
                ...
                **Version simplifiée :**
                ...
                - ✅ Appelle directement vos modules
                - ✅ Interface unifiée
                
                **Exemples de commandes :**
                - `rédiger plainte contre X`
    ...
    if use_simplified:
        # Afficher la version simplifiée
